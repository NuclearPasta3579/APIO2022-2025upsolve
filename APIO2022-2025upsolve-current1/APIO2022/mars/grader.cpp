//This is not the previous sample grader the participants got during contest.
//To make testing better,I've add a srh algorithm to precalculate the answer and compare to the return value.
//Note that 1 is given when the answer is correct,0 otherwise.
	...
	while(t--)
	{
		...
		for(int i = 0; i < 2*n+1; i++)
			for(int j = 0; j < 2*n+1; j++)
				assert(scanf(" %c",&s[i][j]) == 1),ft[i][j] = s[i][j];
		cnt = 0;
		...
		printf("%lld\n",(long long)(to_longlong(h[0][0]) == cnt));
	}
}
